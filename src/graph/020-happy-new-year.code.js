...
const a0 = {
	...
	'Agriculture': {
		'Urban': (fn, sn) => (fn.Urban >= 2 && ((fn.Urban + sn.Urban) >= 6)) || ((fn.Urban + sn.Urban) >= 3 && (fn.Agriculture + sn.Agriculture) >= 4) || ((fn.Urban + sn.Urban) >= 3 && (fn.Grassland + sn.Grassland) >= 6),
	...
	'Grassland': {
		...
		'Urban': (fn, sn) => (fn.Urban >= 3 && ((fn.Urban + sn.Urban) >= 6)) || ((fn.Urban + sn.Urban) >= 3 && (fn.Agriculture + sn.Agriculture) >= 6) || ((fn.Urban + sn.Urban) >= 3 && (fn.Grassland + sn.Grassland) >= 4)
	},
	'Wetland': {},
	'Heathland': {},
	'Littoral': {},
	...
	'Urban': {
		...
		'Rock': (fn, sn) => ((fn.Urban + sn.Urban) >= 12 && fn.Empty == 0 && sn.Empty == 0) || (fn.Rock + sn.Rock) >= 5,
...
const TRESHOLD = 35; // And treshold for color similarity detection
...
const FRAME_RATE = 10; // Speed stuff
const SIMULATION_RATE = 5;
...
function findCategory(pixel) {
	for (const [category, color] of Object.entries(CATEGORIES)) {
		if (distance(color, pixel) < TRESHOLD) return category;
	}
	return 'Missing';
}
...
p.preload = () => {
	img = p.loadImage('https://i.imgur.com/61Zwvci.jpeg'); // XXX selfhost yeah
}
